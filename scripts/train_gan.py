...
def main():
    ...
    generator = Generator(input_dim=args.input_dim).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (x, labels) in enumerate(loader):
            ...
            real_out = discriminator(x + torch.rand_like(x) * 0.1 * args.noise_factor).view(-1)
            ...
            fake_out = discriminator(fake.detach() + torch.rand_like(x) * 0.1 * args.noise_factor).view(-1)
            ...
            # train generator
            generator.zero_grad()
            fake_out = discriminator(fake + torch.rand_like(x) * 0.1 * args.noise_factor).view(-1)
        ...
        # save sample images
        fixed_generated_imgs = generator(fixed_random_noise).detach().cpu()
        ...
        plt.axis('off')
        plt.savefig(f'data/gan_epoch_{epoch + 1}.png')
        plt.close()

        # save model
        if (epoch + 1) % 2 == 0:
            torch.save(generator.state_dict(), f'checkpoints/generator_{epoch + 1}.pt')
            torch.save(discriminator.state_dict(), f'checkpoints/discriminator_{epoch + 1}.pt')
    ...
    plt.legend()
    plt.savefig('data/gan_loss_history.png')
    plt.close()
